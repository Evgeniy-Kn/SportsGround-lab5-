using NameFacility.DomainObjects;
using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using System.Linq;
using Xunit;
using NameFacility.ApplicationServices.GetNameFacilityListUseCase;
using System.Linq.Expressions;
using NameFacility.ApplicationServices.Ports;
using NameFacility.DomainObjects.Ports;
using NameFacility.ApplicationServices.Repositories;

namespace NameFacility.WebService.Core.Tests
{
    public class GetNameFacilityListUseCaseTest
    {
        private InMemoryNameFacilityRepository CreateRoteRepository(InMemoryTransportOrganizationRepository transportOrganizationRepository)
        {
            ...
            var repo = new InMemoryNameFacilityRepository(new List<DomainObjects.NameFacility> {
                new DomainObjects.NameFacility { Id = 1, TypeofSportsGround = "591", AdministrativeDistrict = "Метро \"Войковская\" - Станция Ховрино", Organization = transavtoliz, Type = TransportType.Bus },
                new DomainObjects.NameFacility { Id = 2, TypeofSportsGround = "191", AdministrativeDistrict = "Метро \"Селигерская\" - Станция Ховрино", Organization = transavtoliz, Type = TransportType.Bus },
                new DomainObjects.NameFacility { Id = 3, TypeofSportsGround = "215к", AdministrativeDistrict = "Метро \"Селигерская\" - Станция Ховрино", Organization = mosgortrans, Type = TransportType.Bus },
                new DomainObjects.NameFacility { Id = 4, TypeofSportsGround = "56", AdministrativeDistrict = "Тверская Застава - Базовская улица", Organization = mosgortrans, Type = TransportType.Trolley },
            });
            return repo;
        }
        ...
        public void TestGetOrganizationRoutes()
        {
            ...
            Assert.True(useCase.Handle(GetNameFacilityListUseCaseRequest.CreateNameofSportsgroundNamefacilitiesRequest(1), outputPort).Result);
        ...
        public void TestGetNonExistingOrganizationRoutes()
        {
            ...
            Assert.True(useCase.Handle(GetNameFacilityListUseCaseRequest.CreateNameofSportsgroundNamefacilitiesRequest(999), outputPort).Result);
            Assert.Empty(outputPort.Routes);
        }
    }
    ...
    {
        public IEnumerable<DomainObjects.NameFacility> Routes { get; private set; }
